  ...
  render() {
    return (
      <View style={styles.card}>
        <Text style={{ fontSize: 20, fontWeight: "bold", color: "#F59300" }}>
          {this.props.tweetDate}{" "}
        </Text>
        <Text style={{ fontSize: 18 }}>{this.props.tweetText}</Text>
        {this.props.tweetPicture != null ? (
          <Text
            style={{ fontSize: 18, color: "blue" }}
            onPress={() => Linking.openURL(this.props.tweetPicture)}
          >
            {this.props.tweetPicture}
          </Text>
        ) : null}
        <Image
          source={{ uri: this.props.tweetPicture }}

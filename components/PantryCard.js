    ...
    return (
      <View style={styles.card}>
        ...
        <Text style={{ fontSize: 15 }}>
          ...
          <Text
            style={{ color: "blue" }}
            onPress={() => Linking.openURL(`tel:${this.props.pantry["phone"]}`)}
          >
            {this.props.pantry["phone"]}
          </Text>
        </Text>
        {Object.keys(hours).map(function (key, index) {
          if (
            hours[key]["hours"] != "" &&
            typeof hours[key]["hours"] != "undefined"
          ) {
            if (!hasHours) {
              hasHours = true;
              return [
                ...
                <Text style={{ fontSize: 15 }}>
                  - {hours[key]["frequency"]} {key}: {hours[key]["hours"]}
                </Text>,
              ];
            } else
              return (
        ...
        {Object.keys(services).map(function (key, index) {
          if (services[key] != "" && typeof services[key] != "undefined") {
            ...
            } else return <Text style={{ fontSize: 15 }}>- {key}</Text>;
          }
        })}
        ...
        <Button
          ...
          onPress={() => this._getDirections(this.props.pantry["address"])}
        >
